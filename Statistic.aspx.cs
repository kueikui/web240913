    ...
    public partial class Statistic : System.Web.UI.Page
    {
        ...
        protected void Chart2_Load(object sender, EventArgs e)//跌倒地點
        {
            ...
            else if (Session["LoginType"] == "Home")
            {
                MySqlConnection connection = new MySqlConnection(connectionString);
                connection.Open();
                MySqlCommand command = new MySqlCommand("SELECT HomePlace.hPlace, COUNT(HomeElderFall.hfId) AS FallCount FROM HomeElderFall JOIN HomePlace ON HomePlace.hPlace = HomeElderFall.hPlace GROUP BY HomePlace.hPlace ", connection);
                MySqlDataAdapter da = new MySqlDataAdapter(command);
                DataSet dataset = new DataSet();
                da.Fill(dataset);
        ...
        protected void Chart4_Load(object sender, EventArgs e)//原因
        {
            ...
            else if (Session["LoginType"] == "Home")
            {
                MySqlConnection connection = new MySqlConnection(connectionString);
                connection.Open();
                MySqlCommand command = new MySqlCommand("SELECT hfWhy , COUNT(*) AS FallCount FROM HomeElderFall GROUP BY hfWhy", connection);
                MySqlDataAdapter da = new MySqlDataAdapter(command);
                DataSet dataset = new DataSet();
                da.Fill(dataset);
